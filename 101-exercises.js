...
function removeVowels(string) {
  return string.replace(/A|a|E|e|I|i|O|o|U|u/g, "")
}
...
function startsWithVowel(stringTwo) {
  var stringTwoLower = stringTwo.toLowerCase();
  return stringTwoLower[0] === "a" || 
    stringTwoLower[0] === "e" || 
    stringTwoLower[0] === "i" || 
    stringTwoLower[0] === "o" || 
    stringTwoLower[0] === "u";
...
function endsWithVowel(stringThree) {
  var stringThreeLower = stringThree.toLowerCase();
  var i = stringThree.length - 1;
  return stringThreeLower[i] === "a" ||
    stringThreeLower[i] === "e" ||
    stringThreeLower[i] === "i" ||
    stringThreeLower[i] === "o" ||
    stringThreeLower[i] === "u";
}
...
function startsAndEndsWithVowel(stringFour) {
  var arrayFour = stringFour.split('');
  var VowelArray = ['A', 'a', 'E', 'e', 'I', 'i', 'O', 'o', 'U', 'u'];
  return VowelArray.includes(arrayFour[0]) && VowelArray.includes(arrayFour[(stringFour.length - 1)]);  
}
...
function first(sequence) {
  ...
  if (Array.isArray(sequence)) {
      var firstInArray = sequence[0];
      return firstInArray;
  } else {      
      var seqString = String(sequence);
      var firstLetter = seqString.charAt(0);
      return firstLetter;}     
   }
...
function second(seqTWO) {

  if (Array.isArray(seqTWO)) {
      var SecondInArray = seqTWO[1];
      return SecondInArray;
  } else {      
      var seqString = String(seqTWO);
      var secondLetter = seqString.charAt(1);
      return secondLetter;}     
   }
...
function third(seqTHREE) {

  if (Array.isArray(seqTHREE)) {
      var ThirdInArray = seqTHREE[2];
      return ThirdInArray;
  } else {      
      var seqString = String(seqTHREE);
      var thirdLetter = seqString.charAt(2);
      return thirdLetter;}     
   }
...
function forth(seqFOUR) {

  if (Array.isArray(seqFOUR)) {
      var FourthInArray = seqFOUR[3];
      return FourthInArray;
  } else {      
      var seqString = String(seqFOUR);
      var fourthLetter = seqString.charAt(3);
      return fourthLetter;}     
   }
...
function last(seqLAST) {

  if (Array.isArray(seqLAST)) {
      var i = ((seqLAST.length) - 1);
      var LastInArray = seqLAST[i];
      return LastInArray;
  } else {      
      var seqString = String(seqLAST);
      var lastLetter = seqString.charAt(i);
      return lastLetter;}     
   }
...
function secondToLast(seqNextToLast) {

  if (Array.isArray(seqNextToLast)) {
      var i = ((seqNextToLast.length) - 2);
      var NexttoLastInArray = seqNextToLast[i];
      return NexttoLastInArray;
  } else {      
      var seqString = String(seqNextToLast);
      var i = ((seqNextToLast.length) - 2);
      var NexttolastLetter = seqString.charAt(i);
      return NexttolastLetter;}     
   }
...
function thirdToLast(seqTTL) {

  if (Array.isArray(seqTTL)) {
      var i = ((seqTTL.length) - 3);
      var TTLArray = seqTTL[i];
      return TTLArray;
  } else {      
      var seqString = String(seqTTL);
      var i = ((seqString.length) - 3);
      var TTLLetter = seqString.charAt(i);
      return TTLLetter;}     
   }
...
function firstAndSecond(sequence) {
  ...
  if (Array.isArray(sequence)) {
      var firstInArray = sequence[0];
      var secondInArray = sequence[1];
      var FASArray = [firstInArray, secondInArray];
      return FASArray;
  } else {      
      var seqString = String(sequence);
      var firstLetter = seqString.charAt(0);
      var secondLetter = seqString.charAt(1);
      var FASLetter = [firstLetter, secondLetter];
      return FASLetter;}     
   }
...
function firstAndLast(sequence) {
      var i = ((sequence.length) - 1); // elevating the scope level of var i to minimize lines of code from previous questions.
  if (Array.isArray(sequence)) {
      var firstInArray = sequence[0];
      var lastInArray = sequence[i];
      var FALArray = [firstInArray, lastInArray];
      return FALArray;
  } else {      
      var seqString = String(sequence);
      var firstLetter = seqString.charAt(0);
      var lastLetter = seqString.charAt(i);
      var FALLetter = [firstLetter, lastLetter];
      return FALLetter;}     
   }
...
function median(sequence) {
    var seqRem = sequence.length % 2;
 	  var half = Math.floor(sequence.length / 2);
  if (seqRem == 0) {
  	var med1 = (sequence[half] + sequence[half - 1]) / 2.0;
  	return med1;
} else {
    var med2 = parseFloat(sequence[half]).toFixed(1);
    return parseFloat(med2);
	 }
	}
...
function getHighestNumber(NumSeq) {
  NumSeq.sort(function(a,b) {
    return a - b;})
  var i = NumSeq.length - 1;
  return NumSeq[i];
}
...
function getSmallestNumber(NumSeq) {
    NumSeq.sort(function(b, a) {
    return b - a;})
  return NumSeq[0];
}
...
function onlyOddNumbers(NumSeq) {
  var OddNumArr = [];
  NumSeq.forEach((element) => {
  	var elRem = element % 2;
    if (elRem == 1 || elRem == -1) {
      OddNumArr.push(element);}
  });
    return OddNumArr;
   }
...
function onlyEvenNumbers(NumSeq) {
    var EvenNumArr = [];
  NumSeq.forEach((element) => {
  	var elRem = element % 2;
    if (elRem == 0) {
...
function onlyPositiveNumbers(NumSeq) {
    var PosNumArr = [];
  NumSeq.forEach((element) => {
    if (element > 0) {
      PosNumArr.push(element);}
  });
    return PosNumArr;
 }
...
function onlyNegativeNumbers(NumSeq) {
      var NegNumArr = [];
  NumSeq.forEach((element) => {
    if (element < 0) {
      NegNumArr.push(element);}
  });
    return NegNumArr;
}
...
function hasEvens(NumSeq) {
  ...
  var elRem = element % 2;
    if (elRem == 0) {
      AnyEvenArr.push(element);}
  });
  
    if (AnyEvenArr.length > 0) {
      return true; }
    else { 
      return false; }
  }
...
function countEvens(sequence) {
  ...
  var elRem = element % 2;
    if (elRem == 0) {
      AllEvenArr.push(element);}
  });
    return AllEvenArr.length;
}
...
function hasOdds(sequence) {
  ...
  var elRem = element % 2;
    if (elRem == 1 || elRem == -1) {
      AnyOddArr.push(element);}
  });
    if (AnyOddArr.length > 0) {
      return true; }
    else { 
      return false; }
}
...
function countOdds(sequence) {
  ...
  var elRem = element % 2;
    if (elRem == 1 || elRem == -1) {
      AllOddArr.push(element);}
  });
    return AllOddArr.length;
}
...
// Write a function definition named countNegatives that takes in sequence of numbers and returns a count of the number of negative numbers
 ...
 function countNegatives(sequence) {
  ...
  sequence.forEach((element) => {
    if (element < 0) {
      AllNegArr.push(element);}
  });
    return AllNegArr.length;
}
...
// Write a function definition named countPositives that takes in sequence of numbers and returns a count of the number of positive numbers
 ...
 function countPositives(sequence) {
  ...
  sequence.forEach((element) => {
    if (element > 0) {
      AllPosArr.push(element);}
  });
    return AllPosArr.length;
}
...
function onlyPositiveEvens(NumSeq) {
    var PosEvenArr = [];
  NumSeq.forEach((element) => {
  	var elRem = element % 2;
    if (element > 0 && elRem == 0) {
      PosEvenArr.push(element);}
  });
    return PosEvenArr;
}
...
function onlyPositiveOdds(NumSeq) {
    var PosOddArr = [];
  NumSeq.forEach((element) => {
  	var elRem = element % 2;
    if (element > 0 && (elRem == 1 || elRem == -1)) {
      PosOddArr.push(element);}
  });
    return PosOddArr;
}
...
function onlyNegativeEvens(NumSeq) {
    var NegEvenArr = [];
  NumSeq.forEach((element) => {
  	var elRem = element % 2;
    if (element < 0 && elRem == 0) {
      NegEvenArr.push(element);}
  });
    return NegEvenArr;
}
...
function onlyNegativeOdds(NumSeq) {
    var NegOddArr = [];
  NumSeq.forEach((element) => {
  	var elRem = element % 2;
    if (element < 0 && (elRem == 1 || elRem == -1)) {
      NegOddArr.push(element);}
  });
    return NegOddArr;
}
...
// Write a function definition named shortestString that takes in an array of strings and returns the shortest string in the array.
  ...
  function shortestString(ArrOfStrings) {
	 var shortstring = 
	 ArrOfStrings.reduce(function(a, b) {       //Inline Callback Function & IIFE (Immediately Invoked Function Expression)
			return (a.length <= b.length) ? a : b;   //Ternary Operation (Shorthand for If/Else Statement)
	  })
    return shortstring;
  }
...
function longestString(SeqOfStr) {
	 var longstr = 
	 SeqOfStr.reduce(function(a, b) {  
...
function getUniqueValues(arry) {
  ...
  for (var i = 0; i < arry.length; i++) {
    var istrue = UniqueArry.includes(arry[i]);
    if (istrue == false) {
      UniqueArry.push(arry[i])}
    else { }
}
  return UniqueArry;
}
...
function elementsTimesTwo(NumArry) {
  var TimesTwo = [];
  for (var i = 0; i < NumArry.length; i++) {
    var mult = NumArry[i] * 2;
    TimesTwo.push(mult);
  }
  return TimesTwo;
}
...
function flatten(arr) {
    var superarr = Array.prototype.concat.apply([], arr); //Need to come back to understand "Array.prototype.concat.apply" portion
    return superarr;
}
...
function addOneToArray(arry) {
  var ArrySum = [];
  for (var i = 0; i < arry.length; i++) {
    var arryPlus = arry[i] + 1;
    ArrySum.push(arryPlus);
  }
  return ArrySum;
}
...
function getNumberOfBooks(objArr) {
